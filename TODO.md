...
- [x] using rusqlite, create database.rs, with Database struct that creates if not exists sqlite db, and implements all relevant functions to import, delete symlink items
...
- [x] create cli.rs using Clap that creates a cli with commands sync and import that lets cli user to add a symlink into db by passing its path
